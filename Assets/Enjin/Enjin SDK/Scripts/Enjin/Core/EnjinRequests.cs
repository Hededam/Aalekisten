namespace EnjinSDK
{
    ...
    public class EnjinRequests
    {
        ...
        /// <summary>
        /// Sends a Token on the Trusted Platform and blockchain
        /// </summary>
        /// <param name="identityID">Identity ID of requestor</param>
        ...
        /// /// <param name="value">Callback function to execute when request is fulfilled</param>
        /// <returns>Create request data from API</returns>
        public Request SendItem(int identityID, string tokenID, int recipientID, int value, System.Action<RequestEvent> callback)
        {
            Request request = this.SendItem(identityID, tokenID, recipientID, value);
            int requestID = request.id;
            Enjin.RequestCallbacks.Add(requestID, callback);
            return request;
        }
